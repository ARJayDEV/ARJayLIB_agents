	...
	{		
		...
		do
		{
			...
			if(cursorTarget isKindOf "House") then
			{
				if(_hold > 1) then
				{
					[cursorTarget] call arjay_debugBuildingPositions;
					_hold = 0;
				};
...
arjay_timerStart = 
{
	...
	["%1 [TIMER STARTED]",_text] call arjay_dump;	
};
...
arjay_timerStop = 
{
	...
	["%1 [TIMER ENDED : %2]",_text,_endTimer] call arjay_dump;	
};
...
arjay_inspect = 
{
	...
	_text = format[" unit ready: %1", unitReady _target];
	[_text] call arjay_dumpOutput;
	...
	*/
	...
	_text = format[" empty positions: %1", [_target] call arjay_getEmptyVehiclePositions];
	[_text] call arjay_dumpOutput;
	...
	*/
	...
	if(count (_target getVariable ["arjay_agentProfile", []]) > 0) then 
	{
		_text = format[" agent profile: %1", _target getVariable "arjay_agentProfile"];
		[_text] call arjay_dumpOutput;
		
		_text = format[" agent busy: %1", _target getVariable "arjay_agentBusy"];
		[_text] call arjay_dumpOutput;		
	};
...
arjay_inspectConfigItem = 
{
	...
	if(isClass _item) then
	{
		...
		if(_detailed) then
		{
			["class: %1 type: %2 scope: %3 picture: %4 path: %5",_class,_type,_scope,_picture,_item] call arjay_dumpOutput;
		}
		else
		{
			["%1",_class] call arjay_dumpOutput;
		};
